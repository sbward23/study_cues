---
title: "Variable construction and model preparation"
output: 
  html_document:
    toc: true 
    toc_depth: 4
...
```{r}
d <- read_csv(here::here(path_ready, "imputed_1.csv"), show_col_types = FALSE)
```
...
d <- d %>%
  mutate(
    ...
    prec = factor(prec,
                  levels = c(0, 1),
                  labels = c("No_history", "Precedent")),
    
    bpa  = factor(bpa,
                  levels = c(0, 1),
                  labels = c("No_venues", "Bar_party")),
...
# Drinks if ordered factor (likely untenable?)
d <- d %>%
  mutate(
    drk_ord = factor(
      drink,
...
v_m1 <- lmer(verb_yj ~ day_c + (1 + day_c | nid), data = d_lmm, REML = TRUE)
...
summary(v_m1_clmm)
as.data.frame(VarCorr(v_m1_clmm))
```

Evaluate study `day` for inclusion in **nonverbal** models as fixed
design covariate and assess support for random slope

```{r}
n_null <- lmer(nvrb_yj ~ 1 + 
                 (1 | nid), data = d_lmm, REML = FALSE)

n_m0 <- lmer(nvrb_yj ~ day_c +
               (1 | nid), data = d_lmm, REML = FALSE) # compare to unconditional LMEM

anova(n_null, n_m0) # support for fixed design covariate? No

n_m1 <- lmer(nvrb_yj ~ day_c + (1 + day_c | nid), data = d_lmm, REML = FALSE)

anova(n_m0, n_m1) # support for random slope? No
...
test_random_slope(
  outcome = "verb_yj",
  wp = "prec_precedent_wp",
  pm_c = "prec_precedent_pm_c",
  data = d_lmm
)
```
...
saveRDS(
  d_lmm,
  here::here(path_ready, "imp1_ana.rds"))
```
