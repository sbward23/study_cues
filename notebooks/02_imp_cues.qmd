...
```{r}
b <- read_csv(here::here(path_ready, "base_tst.csv"), show_col_types = FALSE)

diary <- read_csv(here::here(path_ready, "diary_tst.csv"), show_col_types = FALSE)
```
...
The diary are structured in long format, with one row per event and
multiple events nested within participants. A majority of participants
(60.5%) contributed more than one event, with a median of two events per
participant and up to 19 events for some individuals.
...
obs was coded NA to be imputed

```{r}
colSums(is.na(df))[colSums(is.na(df)) > 0]
```

A visual and tabular inspection of missing data patterns indicated that
missingness was sparse, localized to a small number of event-level
variables, and not concentrated within specific participants or events.

```{r}
df_subset <- df %>%
  select(bpa, fm_r, drink, verb, enon, inon, prec, aud2)

md.pattern(df_subset)
```

The majority of observations were complete, and no observation exhibited
extensive missingness across variables. Given this pattern, multiple
imputation with chained equations will be used to address missing data
under a missing-at-random assumption.

```{r}
aggr(df, numbers = TRUE)

aggr(df_subset, numbers = TRUE)
```
...
saveRDS(
  completed_list,
  here::here(path_ready, "imputed_datasets.rds"))
```
...
purrr::iwalk(
  completed_list,
  ~ write_csv(.x, here::here(path_ready, paste0("imputed_", .y, ".csv")))
)
```
