...
# checked 9128298 for aud1:aud10, aub module, and psb module; nid does not drink
# d <- d %>%
#  mutate(bpa = if_else(nid == 9128298 & bpa == 3, 0, bpa))
...
d <- d %>%
  mutate(across(
    c(au_pg, au_dg, psb3, prec, psb5a, er1_p, er2_p, 
...
```{r}
psych::describe(d$inon) # implicit nonverbal
```
...
`sact`: What activities occurred? (check all that apply)
...
```{r}
d %>% select(sact) %>% map(tabyl)

d <- d %>%
  mutate(sact = as.character(sact),
         sact = case_when(
           sact %in% c("1", "1,2", "2") ~ "Contact",
           sact == "7" ~ "Attempt",
           grepl("3", sact) | grepl("4", sact) | 
             grepl("5", sact) | grepl("6", sact) ~ "Penetration",
           TRUE ~ as.character(sact)))
...
`drnk`: How many drinks did you consume prior to or during this activity?
...
d %>% select(drnk) %>% map(tabyl)
```

-   Recode `psb_alc` from `drnk` such that zero drinks = 0 and \>0 drinks = 1

```{r}
d <- d %>%
  mutate(psb_alc = if_else(drnk == 1, 0, if_else(drnk > 1, 1, drnk)))

d %>% select(psb_alc) %>% map(tabyl)
```

Participants endorsed `r nrow(subset(d, psb_alc == 1))` partnered sexual events 
following alcohol consumption.

### Self-reported intoxication

`s_tx`: How intoxicated did you feel right before or during this sexual activity?

Least intoxicated ever felt in life (0) - Most intoxication ever felt in life (100) VAS

```{r}
psych::describe(d$s_tx)
...
`p_tx`: How intoxicated do you think the other person felt right before or during this sexual activity?
...
d$parity_con <- abs(d$s_tx - d$p_tx)
hist(d$parity_con)
...
d <- d %>%
  mutate(
    parity_cat = case_when(
      parity_con == 0 ~ "Equal",
      s_tx > p_tx   ~ "Self-Higher",
      s_tx < p_tx   ~ "Partner-Higher",
      TRUE ~ NA_character_
    ),
